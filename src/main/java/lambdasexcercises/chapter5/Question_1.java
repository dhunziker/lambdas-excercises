...
public class Question_1 {
	...
	public int count1(List<Integer> numbers) {
		return numbers.stream().reduce(0, (acc, elem) -> acc + elem);
	}

	// Using method reference
	public int count2(List<Integer> numbers) {
		return numbers.stream().reduce(0, Integer::sum);
	}

	public List<Integer> concat1(List<Integer> left, List<Integer> right) {
		return Stream.of(left, right).flatMap(numbers -> numbers.stream()).collect(toList());
	}

	// Using method reference
	public List<Integer> concat2(List<Integer> left, List<Integer> right) {
